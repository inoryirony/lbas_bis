...
- 静态 Rank-1 使用容量支配与精确基地前沿搜索；两基地详细 Rank-1 使用四 worker 的后缀前沿和共享 incumbent，其他详细模式与 Top K 使用分组 branch-and-bound，并区分“模型内已证明最优”和“当前固定样本集内已证明最优”。
- 小规模随机场景会与独立穷举 oracle 对拍，防止剪枝删除可行解或最优解。
- 详细战斗解析敌方 HP、装甲、目标选择、命中、暴击、擦弹、击沉、旗舰保护、普通触接、陆侦倍率、潜艇/东海、PT/特殊敌和活动装备倍率；沉船不会在后续波次复活。
...
详细搜索的 `provenOptimal: true` 只证明结果在当前 seed 对应的固定样本集合上最优，不等于已经证明真实无限样本期望最优。启发式只用于尽快找到当前最好方案，不会删除候选；参与最终证明的剪枝必须是数学安全的上界或硬约束。搜索会先发布可行方案，再继续在后台证明；导入当前方案不会取消搜索。
...
未知总工作量阶段使用插件自身按流式耗时驱动的活动条；已知总工作量阶段显示精确百分比并保留活动标记。两种状态都会持续显示节点、剪枝、完整候选、模拟样本和耗时，避免长证明期间看起来像卡死。

## 已知限制

- 敌方只有总制空时无法连续削减敌机，必须使用静态估算。
- 地图预设敌舰数据完整时，详细模式会模拟敌舰队对陆航的 Stage 2、装备抗击坠和喷式强袭 Stage 2；敌方对空 CI、我方舰队对敌机的普通 Stage 2 等仍未模拟。
- 命中、回避、触接等少数常数来自成熟模拟器的经验模型，并非游戏官方公开公式；固定样本最优也不能外推成无限样本总体最优。
...
9. 点击 `计算优化`。首个可行方案会先显示，搜索继续在后台证明最优；可导入当前方案而不中断证明，也可随时停止并保留当前方案。
...
lbas-bis simulate --scenario scenario.json
...
`simulate` 优先模拟场景中的 `bases`；未提供时会模拟 `lockedBases`，并按 `instanceId` 从场景装备或 `--poi` 库存解析飞机。输出包含逐波制空、敌机削减、`expectedHpDamage` 与 `expectedSunkCount`；敌舰 HP 或装甲不完整时，真实伤害字段为 `null` 并返回明确限制码。
...
- `src/combat-frontier-solver.js`：详细 Rank-1 后缀前沿、继续状态和安全战斗上界。
- `src/search-runner.js`：worker 生命周期、四分片聚合、取消和替换搜索。
